...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    ...
    path: `/blog/hotels/compare/${params.resort}`,
  ...
  return (
    <>
      <JsonLd data={buildHotelListSchema(hotels.map(h => ({
        name: h.name, url: `${siteUrl}/blog/hotels/compare/${params.resort}`,
        image: h.image, rating: h.rating, reviewCount: h.reviewCount,
      })))} />
      <JsonLd data={buildBreadcrumbSchema([
        { name: 'Главная', url: siteUrl },
        { name: dest.name, url: `${siteUrl}/blog/destinations/${dest.countrySlug}/${dest.slug}` },
        { name: `Сравнение отелей ${dest.name}`, url: `${siteUrl}/blog/hotels/compare/${params.resort}` },
      ])} />

      <Breadcrumbs items={[
        { label: 'Куда поехать', href: '/blog/kuda' },
        { label: dest.name, href: `/blog/destinations/${dest.countrySlug}/${dest.slug}` },
      ...
      <div className="mt-8 p-5 bg-gray-50 rounded-2xl border border-gray-200">
        ...
        <div className="flex flex-wrap gap-3">
          <Link href={`/blog/destinations/${dest.countrySlug}/${dest.slug}`} className="text-accent text-sm hover:underline">
            ← Обзор курорта {dest.name}
          </Link>
          <Link href="/blog/tips/general/egypt-tips-2026" className="text-accent text-sm hover:underline">
            Лайфхаки для отдыха →
          </Link>
        </div>
      </div>
